...
```sh
for run in experiments/sweeps/recurrence_grid/results/seed1337 experiments/sweeps/recurrence_grid/results/seed1338; do
...
At the final checkpoint, evaluate `(0,0)`, `(3,0)`, `(0,3)`, and `(3,3)` with identical prompts and sampling seeds. Save the JSON prompt list `[';1.', ';1.e4 e5 2.', ';1.d4 d5 2.', ';1.e4 c5 2.']` with proper JSON double quotes to `experiments/sweeps/recurrence_grid/results/prompts.json`. For each run and pair, use the following command with the appropriate counts and a distinct output filename:

```sh
uv run python sample.py --checkpoint experiments/sweeps/recurrence_grid/results/seed1337/ckpt-step000100.pt \
  --device=mps --execution=training_graph --u-t=3 --u-d=3 --mask-seed=11 \
  --prompts experiments/sweeps/recurrence_grid/results/prompts.json --num-samples=20 --seed=2027 \
  --temperature=1 --top-k=0 --max-new-tokens=128 \
  --output experiments/sweeps/recurrence_grid/results/seed1337/generation-3-3.json
```
