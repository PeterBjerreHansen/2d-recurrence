...
Use the existing full `data/chess_long_v1` dataset (143,017 train rows, 1,445 validation rows) and unchanged `experiments/long_runs/baseline/results/panels.json` (128 selection rows, the remaining 1,317 confirmation rows). Selection evaluation covers all nine recurrence cells and all 13 distinct pilot mask placements. Do not regenerate a smaller dataset or resample the panel.
