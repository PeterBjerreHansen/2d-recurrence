...
Follow-up exact-row evaluation, feedback-content intervention, and the unchanged 1,000-update pilot are documented in Stage 9 continued results (historical path: `../../pilots/recurrence/REPORT.md`).
...
The training configurations, package versions, manifest hash, and checkpoint provenance are retained in seed 1337 `run.json` (historical path: `results/seed1337/run.json`) and seed 1338 `run.json` (historical path: `results/seed1338/run.json`). The initial preflight passed all 50 tests and verified `torch.backends.mps.is_available() == True`.

## Retained checkpoints and training curves

Each seed retained checkpoints at optimizer steps 0, 25, 50, 75, and 100, plus the latest `ckpt.pt`. The intermediate snapshots are seed 1337 step 0 (historical path: `results/seed1337/ckpt-step000000.pt`), 25 (historical path: `results/seed1337/ckpt-step000025.pt`), 50 (historical path: `results/seed1337/ckpt-step000050.pt`), 75 (historical path: `results/seed1337/ckpt-step000075.pt`), 100 (historical path: `results/seed1337/ckpt-step000100.pt`), and the corresponding seed 1338 step 0 (historical path: `results/seed1338/ckpt-step000000.pt`), 25 (historical path: `results/seed1338/ckpt-step000025.pt`), 50 (historical path: `results/seed1338/ckpt-step000050.pt`), 75 (historical path: `results/seed1338/ckpt-step000075.pt`), 100 (historical path: `results/seed1338/ckpt-step000100.pt`).
...
!Validation NLL versus optimizer updates (historical path: `results/curves/nll-vs-updates.png`)

!NLL difference versus ordinary execution (historical path: `results/curves/delta-nll-vs-00.png`)

The reproducible tables are per-seed grid curves (historical path: `results/curves/grid-curves-by-seed.csv`), seed-summary curves (historical path: `results/curves/grid-curves-seed-summary.csv`), and placement-level results (historical path: `results/curves/placement-results.csv`). The small plotting/aggregation helper (historical path: `curves.py`) reads the raw JSON reports and writes these tables and plots.
...
The evaluator ran one fixed-batch eval-mode diagnostic per cell and retained the traces in every grid JSON and in diagnostics.csv (historical path: `results/curves/diagnostics.csv`). Activations and gradients were finite. Representative final-step values are below; RMS entries are listed by core/source invocation in pass order.
...
All rows had PGN parse success rate 0 and valid-termination rate 0 under the 128-token limit. The small sample and seed spread do not establish a generation win. The ordinary checkpoint is the existing 100-update baseline checkpoint (historical path: `../../smoke/results/baseline-pilot/ckpt.pt`), evaluated only as a contextual reference; its training validation metric was NLL 1.7267 / accuracy 0.400 at step 100, but it was not evaluated by the recurrent grid. Its run metadata is here (historical path: `../../smoke/results/baseline-pilot/run.json`).

Raw generation reports are ordinary baseline (historical path: `../../smoke/results/baseline-pilot/generation-stage09.json`), seed 1337 `(0,0)` (historical path: `results/seed1337/generation-0-0.json`), seed 1337 `(3,0)` (historical path: `results/seed1337/generation-3-0.json`), seed 1337 `(0,3)` (historical path: `results/seed1337/generation-0-3.json`), seed 1337 `(3,3)` (historical path: `results/seed1337/generation-3-3.json`), seed 1338 `(0,0)` (historical path: `results/seed1338/generation-0-0.json`), seed 1338 `(3,0)` (historical path: `results/seed1338/generation-3-0.json`), seed 1338 `(0,3)` (historical path: `results/seed1338/generation-0-3.json`), and seed 1338 `(3,3)` (historical path: `results/seed1338/generation-3-3.json`). The aggregate generation summary CSV (historical path: `results/curves/generation-summary.csv`) retains attempted-move and termination counts.
...
| ---: | --- | --- | --- | --- | --- |
| 1337 | JSON (historical path: `results/seed1337/grid-step000000.json`) / CSV (historical path: `results/seed1337/grid-step000000.csv`) | JSON (historical path: `results/seed1337/grid-step000025.json`) / CSV (historical path: `results/seed1337/grid-step000025.csv`) | JSON (historical path: `results/seed1337/grid-step000050.json`) / CSV (historical path: `results/seed1337/grid-step000050.csv`) | JSON (historical path: `results/seed1337/grid-step000075.json`) / CSV (historical path: `results/seed1337/grid-step000075.csv`) | JSON (historical path: `results/seed1337/grid-step000100.json`) / CSV (historical path: `results/seed1337/grid-step000100.csv`) |
| 1338 | JSON (historical path: `results/seed1338/grid-step000000.json`) / CSV (historical path: `results/seed1338/grid-step000000.csv`) | JSON (historical path: `results/seed1338/grid-step000025.json`) / CSV (historical path: `results/seed1338/grid-step000025.csv`) | JSON (historical path: `results/seed1338/grid-step000050.json`) / CSV (historical path: `results/seed1338/grid-step000050.csv`) | JSON (historical path: `results/seed1338/grid-step000075.json`) / CSV (historical path: `results/seed1338/grid-step000075.csv`) | JSON (historical path: `results/seed1338/grid-step000100.json`) / CSV (historical path: `results/seed1338/grid-step000100.csv`) |

The prompts are saved as proper JSON in experiments/sweeps/recurrence_grid/results/prompts.json (historical path: `results/prompts.json`). No checks were started for grid expansion, extensive component baselines, hyperparameter search, live-feedback inference, or later stages.
