...
def max_update_distribution(matrix):
...
@pytest.mark.parametrize('name, mode, scheduled', [
    ...
    ('depth_hard_1to3', 'depth', True),
    ('temporal_fixed', 'temporal', False),
    ('temporal_hard_1to3', 'temporal', True),
    ('hybrid_fixed', 'hybrid', False),
    ('hybrid_hard_1to3', 'hybrid', True),
])
def test_one_billion_study_arms_resolve_expected_schedule(name, mode, scheduled):
    ...
    assert (config['update_probability_schedule'] is not None) is scheduled
    if scheduled:
        assert update_probabilities_at_step(config, 8309) != update_probabilities_at_step(config, 8310)
        assert config['update_probability_schedule']['phases'][1]['start_step'] == 8310
    else:
        assert update_probabilities_at_step(config, 0) == update_probabilities_at_step(config, 8310)
...
def test_temporal_only_control_uses_temporal_axis_and_expected_evaluation():
    ...
    assert max_update_distribution(config['update_probabilities']) == pytest.approx(
        study.FIXED_UPDATE_PROBABILITIES)
    assert all(row[1:] == (0.0, 0.0) for row in config['update_probabilities'])


@pytest.mark.parametrize('name', ['depth_fixed', 'temporal_fixed', 'hybrid_fixed'])
def test_fixed_arms_match_update_count_distribution(name):
    matrix = study.run_config(name)['update_probabilities']
    assert max_update_distribution(matrix) == pytest.approx(study.FIXED_UPDATE_PROBABILITIES)


@pytest.mark.parametrize('name', ['depth_hard_1to3', 'temporal_hard_1to3', 'hybrid_hard_1to3'])
def test_hard_arms_match_fixed_update_count_distribution_over_horizon(name):
    schedule = study.run_config(name)['update_probability_schedule']['phases']
    weights = (study.CROSSOVER_STEP / study.UPDATES,
               (study.UPDATES - study.CROSSOVER_STEP) / study.UPDATES)
    phase_distributions = [max_update_distribution(phase['update_probabilities']) for phase in schedule]
    actual = tuple(sum(weight * distribution[index]
                       for weight, distribution in zip(weights, phase_distributions))
                   for index in range(len(UPDATE_SUPPORT)))
    assert actual == pytest.approx(study.FIXED_UPDATE_PROBABILITIES)
...
def test_source_snapshot_excludes_unrelated_experiment_receipts():
    ...
    assert 'experiments/ablations/1B_update_schedule/study.py' in paths
...
def test_one_billion_config_entry_points_work_with_configurator(name, monkeypatch):
    ...
    config = (Path('experiments/ablations/1B_update_schedule/configs') / f'{name}.py')
    ...
    assert namespace['out_dir'].startswith('experiments/ablations/1B_update_schedule/')
